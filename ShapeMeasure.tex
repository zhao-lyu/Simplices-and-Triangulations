\subsection{Shape Regularity Measure}%\mbox{}\\
    $\emph{Shape measure}$ offers an objective mathematical measure on the overall quality of a simplex, and this is helpful to explore the simplex regularity and to improve the quality of shapes of the elements. Different definitions are used for shape measure to present the quality of simplex, and we simply introduce the $\emph{geometric shape measure} ~\mu({T})$ of simplex ${T}$,  the one we use in this paper.
    ...
    Simplex diameter and volume are important to introduce shape measures. Here we define the $\emph{shape measure}$ $\mu(T)$ of a $k$-simplex $T$ by
    \begin{equation*}
